...
int distributereceive(int f, int c, int *m, int *x, int size) {
	...
	*/
	int countRank = 1, stopProcess = 1;
	int filaMatriz[c];
	MPI_Status Stat;

	for(int i=0; i<f; i++){
		for(int j=0; j<c; j++){
			filaMatriz[j] = *(m+i*MAXCOL + j);
		}

		if(countRank >= size){
			countRank = 1;
		}else{
			MPI_Send(filaMatriz,c,MPI_INT,countRank,TAGTAREA,MPI_COMM_WORLD);
			MPI_Recv(&x[countRank], 1, MPI_INT, countRank, MPI_ANY_TAG, MPI_COMM_WORLD, &Stat);
	...
	for(int x=1; x<size; x++){
		MPI_Send(&stopProcess,1,MPI_INT,x,TAGPARAR,MPI_COMM_WORLD);
	}
...
int receive(int rank) {
	...
	*/
	int vectorV[MAXCOL], vectorA[MAXCOL];
	int result = 0, multp = 0, stop=0;
	MPI_Status Stat;

	MPI_Recv(&vectorV, MAXCOL, MPI_INT, 0, TAGVECTOR, MPI_COMM_WORLD, &Stat);
	MPI_Recv(&vectorA, MAXCOL, MPI_INT, 0, TAGTAREA, MPI_COMM_WORLD, &Stat);

	if(Stat.MPI_TAG == TAGTAREA){
		for(int i=0; i<MAXCOL; i++){
			multp = vectorV[i] * vectorA[i];
			result += multp;
		}
		MPI_Send(&result, 1, MPI_INT, 0, TAGRESULT, MPI_COMM_WORLD);
	}else if(Stat.MPI_TAG == TAGPARAR){
		printf("El proceso: %d ha dejado de procesar por orden del maestro", rank);
	}
